...
void PlayListModel::trackChanged(DB_playItem_t *from, DB_playItem_t *to) {
    if (status_column != -1) {
        ddb_playlist_t *plt = DBAPI->plt_get_curr();
        emit dataChanged(createIndex(0, status_column), createIndex(DBAPI->plt_get_item_count(plt, PL_MAIN) - 1, status_column));
        DBAPI->plt_unref(plt);
    }
}
...
void PlayListModel::playerPaused() {
    ...
    ddb_playlist_t *plt = DBAPI->plt_get_curr();
    QModelIndex index = createIndex(DBAPI->plt_get_item_idx(plt, track, PL_MAIN), status_column);
...
void PlayListModel::sort(int column, Qt::SortOrder order) {
    ...
    emit dataChanged(createIndex(0, 0), createIndex(DBAPI->plt_get_item_count(plt, PL_MAIN), columns.count()));
    DBAPI->plt_unref(plt);
}
